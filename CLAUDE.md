...
HTML 쪽 탭 블록은 `<!-- ══ ... TAB ══ -->` 주석으로 나뉘어 있고, **탭 버튼에 보이는 순서와 같은 순서로** 놓여 있다(뉴스 → Fun Facts → 스팀 → 자막&표정 → 북마크 → 저장된 컨텐츠). 탭을 추가하면 이 순서를 유지한다.
...
| 📦 저장된 컨텐츠 | 생성한 컨텐츠 보관·열람. 백업 내보내기/복원도 여기 | 없음 |
...
**백업** — 저장된 컨텐츠 탭의 `⬇ 백업` / `⬆ 복원`(`bk*` 함수). `px_bookmarks` + `px_saved` + `px_ledy_prompt`를 JSON 한 파일로 내보내고 되돌린다.

API 키 두 개는 일부러 뺐다. 백업 파일에 키가 들어가면 그 파일이 곧 유출 경로가 되고, 키는 다시 넣으면 그만이다. 뉴스 캐시도 뺐다 — 다시 받으면 되는 데이터다.

복원은 병합이 아니라 **덮어쓰기**다(확인창 있음). 병합은 중복 판정 규칙이 필요해서 미뤘다. 복원 직후 `location.reload()`로 페이지를 새로 그린다 — 메모리에 있는 `bookmarks` / `savedContents` / Ledy 프롬프트 textarea를 일일이 다시 채우는 것보다 이쪽이 짧고 확실하다.
...
**버전은 사용자가 요청할 때만 올린다.** 기능을 추가했다는 이유로 알아서 올리지 않는다. 2.0.0은 이관과 정리가 다 끝났다고 사용자가 판단하는 시점에 붙는다. 그 뒤로는 semver를 따른다 — 기능 추가는 마이너(2.1.0), 수정은 패치(2.0.1). 버전을 올릴 때 날짜도 같이 갱신한다.
...
- **`localStorage.setItem` 호출 11곳 전부에 try/catch가 없다.** 도메인당 한도(약 5MB)를 넘기면 예외가 나면서 저장이 조용히 중단된다. 컨텐츠 1건이 5~15KB라 수백 건은 들어가므로 급하진 않다. 복원 경로(`bkImport`)에만 try/catch를 넣어뒀다 — 거기서 실패하면 손실이 크기 때문이다.
- Ledy 눈 깜빡임 프레임 자동 삽입 (아래 8-9 참조)
- Reddit 트렌딩, 게임 출시 일정 위젯

### 해결됨

- ~~백업 수단 없음~~ → `bk*` 함수로 JSON 내보내기/복원 구현
- ~~`generateFromFree()` 중복 정의~~ → 죽은 쪽 삭제
- ~~`STEAM_DL_DIR` 개인 PC 경로 하드코딩~~ → `$env:USERPROFILE\Downloads`로 교체
